...
const allowedOrigins = [
  ...
  'https://homecarenote-qoa5ryxfm-jefferson-davids-projects.vercel.app'
].filter(Boolean);

const corsOptions = {
  origin: (origin, callback) => {
    if (!origin) {
      return callback(null, true); // Allow server-to-server or tools without origin
    }

    const normalizedOrigin = origin.replace(/\/$/, '');
    if (allowedOrigins.includes(normalizedOrigin) || process.env.ALLOW_ALL_ORIGINS === 'true') {
